...
function scoreSetup(sessionLabel, sessionOk, sweep, displacement, bos, pullback,
  ...
  // ── H1 STRUCTURAL BIAS (+5 aligned / -20 counter) ────────────
...
app.get('/analyze/:sym', async (req, res) => {
  ...
  // H1 structural bias — fetched separately, same cache as autoScan
...
// ── H1 STRUCTURAL BIAS ───────────────────────────────────────────────────────
// True HTF reference for gold. 30 H1 candles (~30 hours) — wide enough to
// capture daily structure. Tuned for H1: 2-side pivot detection, wider BOS
// margin (0.05%) to avoid false breaks from H1 bar wicks.
function calcH1Bias(h1Candles) {
  ...
  const BOS_MARGIN = 0.0005; // wider for H1 bar noise
  let lastBOS = 'NONE';
  const recent = candles.slice(-5);
  for (const c of recent) {
    if (c.c > lastHigh * (1 + BOS_MARGIN)) { lastBOS = 'UP';   break; }
  ...
  if      (lastBOS === 'UP'   && (isHHHL || aboveMid))  { bias = 'BULLISH'; reason = 'H1 BOS UP + '   + (isHHHL ? 'HH/HL' : 'above mid'); }
  else if (lastBOS === 'DOWN' && (isLHLL || !aboveMid)) { bias = 'BEARISH'; reason = 'H1 BOS DOWN + ' + (isLHLL ? 'LH/LL' : 'below mid'); }
  else if (isHHHL && aboveMid)  { bias = 'BULLISH'; reason = 'H1 HH/HL + above mid (no BOS yet)'; }
  else if (isLHLL && !aboveMid) { bias = 'BEARISH'; reason = 'H1 LH/LL + below mid (no BOS yet)'; }
  else                          { bias = 'NEUTRAL';  reason = 'H1 ranging / choppy'; }
...
async function autoScan() {
  ...
  for (const sym of Object.keys(setups)) { // All active symbols: XAUUSD, WTIUSD
    try {
      ...
      // Cached 5min like M5, so no extra API rate pressure.
      ...
      // H1 is the true HTF reference. Counter-H1 setups are blocked below
      // zone score 80. If ≥ 80, they pass but require 1.8× displacement.
      ...
      if (isCounterH1) {
        console.log('[h1-gate] ' + sym + ': counter-H1 (' + _h1BiasNow + ') — zone ' +
          zoneScore + '/100 ≥ 80, applying 1.8× displacement requirement');
      }
      if (isAlignedH1) {
        console.log('[h1-gate] ' + sym + ': H1 ALIGNED (' + _h1BiasNow + ') — full engine enabled');
      }

      // dispMinRatio: counter-H1 = 1.8×, counter-M15 = 1.2×, normal = function default
      const dispMinRatio = isCounterH1 ? 1.8 : _htfCounter ? 1.2 : null;
